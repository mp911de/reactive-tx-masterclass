...
@SpringBootTest(properties = "spring.data.mongodb.uri=mongodb://localhost/database?replicaSet=rs0&retryWrites=false",
		classes = ReactiveSpringDataMongoTransactionTests.TestConfiguration.class)
final class ReactiveSpringDataMongoTransactionTests {
	...
	void autoCommitWithFailure(@Autowired ReactiveMongoOperations operations) {
		...
		Mono<Document> insertDeleteEvent = operations.insert(
				new Document("_id", 1).append("firstName", "Jesse").append("lastName", "Pinkman").append("ACTION", "DELETED"),
				"personEvent");

		removePerson.then(insertDeleteEvent).then().as(StepVerifier::create).verifyError(DuplicateKeyException.class);
	...
	void programmaticTransactionalWithRollback(@Autowired ReactiveMongoOperations operations,
		...
		insertPerson.then(insertEvent).then().as(rxtx::transactional).as(StepVerifier::create).verifyComplete();
		...
		removePerson.then(insertDeleteEvent).then().as(rxtx::transactional).as(StepVerifier::create)
	...
	void atTransactionalWithRollback(@Autowired ReactiveMongoOperations operations,
		...
		transactionalService.delete().as(StepVerifier::create).verifyError(DuplicateKeyException.class);
